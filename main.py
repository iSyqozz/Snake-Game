...
def run_game(root,start_menu,speed):
    ...
    def move(canvas,q,curr_dir,mat,root,speed,empty,score,job):
        ...
        if canvas.itemcget(mat[next_slot[0]][next_slot[1]],'fill') == '#ad0f07':
            ...
            recursive_delete(game_frame)
        if not gameover:
            job[0] = root.winfo_children()[1].after(speed, lambda: move(canvas,q,curr_dir,mat,root,speed,empty,score,job))
...
def recursive_delete(game_frame):
    for val in game_frame.winfo_children():
        recursive_delete(val)
    game_frame.destroy()
    del game_frame 
